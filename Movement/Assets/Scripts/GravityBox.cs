...
public class GravityBox : GravitySource {
	...
	public override Vector3 GetGravity(Vector3 position) {
		position = transform.InverseTransformDirection(position - transform.position); // Get local position transform and invert its direction
		...
		if (outside > 0) {
			float distance = outside == 1 ? Mathf.Abs(vector.x + vector.y + vector.z) : vector.magnitude; // If we're directly on top of one face, we can use the non-zero vector component to determine distance
